...
from pacman.operations.rigged_algorithms.geometry import \
    ...
    shortest_torus_path, longest_dimension_first, Routes, Links
...
class NerRoute(object):
    ...
    def __call__(self, vertices_resources, nets, machine, constraints, placements,
                 machine_graph, allocations={}, core_resource=None, radius=20):
        ...
        for net in nets:
            ...
            root, lookup = self.ner_net(placements[net.source],
            ...
            if self.route_has_dead_links(root, machine):
                root, lookup = self.avoid_dead_links(root, machine, wrap_around)
    ...
    def memoized_concentric_hexagons(self, radius):
        ...
        large networks.
        """
        out = self._concentric_hexagons.get(radius)
        if out is None:
            out = tuple(concentric_hexagons(radius))
            self._concentric_hexagons[radius] = out
        return out

    def ner_net(self, source, destinations, width, height,
                wrap_around=False, radius=10):
        ...
        for destination in sorted(destinations,
            ...
            concentric_hexagons = self.memoized_concentric_hexagons(radius)
    ...
    def a_star(self, sink, heuristic_source, sources, machine, wrap_around):
    ...
    def avoid_dead_links(self, root, machine, wrap_around=False):
        ...
        root, lookup, broken_links = self.copy_and_disconnect_tree(root, machine)
